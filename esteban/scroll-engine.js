...
function mountScrollWorld(container, config) {
  ...
  function raf() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // Snappier catch-up on touch: a finger drag expects the video to track
      // it closely; the slower 0.18 desktop ease (tuned for mouse-wheel) reads
      // as lag when scrubbing by hand.
      s.cur += (s.target - s.cur) * (reduce ? 1 : (isMobile() ? 0.34 : 0.18));
